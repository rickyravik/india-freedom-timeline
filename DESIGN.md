...
| **Performance** | Fine (static, code-split), but chunk naming was accidental. | Still zero animation libraries: reveals via one IntersectionObserver, scroll-linked effects via CSS `animation-timeline` with static fallback. Fonts self-hosted, variable Fraunces replaces four static cuts. |
